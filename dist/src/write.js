...
function findAlerts(curSuite, prevSuite, prevBest, threshold, compareWithBest) {
    if (compareWithBest) {
...
function buildComment(benchName, curSuite, prevSuite, prevBest, compareWithBest, config) {
...
function buildAlertComment(alerts, benchName, curSuite, prevSuite, prevBest, threshold, compareWithBest, cc, config) {
    ...
    for (const alert of alerts) {
        ...
        const line = `| \`${current.name}\` | ${strVal(best)} | ${strVal(prev)} ` +
...
async function handleComment(benchName, curSuite, prevSuite, prevBest, config) {
...
async function handleAlert(benchName, curSuite, prevSuite, prevBest, config) {
    const { alertThreshold, githubToken, commentOnAlert, failOnAlert, alertCommentCcUsers, failThreshold, compareWithBest, } = config;
    ...
    const body = buildAlertComment(alerts, benchName, curSuite, prevSuite, prevBest, alertThreshold, compareWithBest, alertCommentCcUsers, config);
    let message = body;
    let url = null;
    if (commentOnAlert) {
        if (!githubToken) {
...
async function writeBenchmark(bench, config) {
    ...
    const prevBench = prevSuites[prevSuites.length - 1];
    // Get best benchmark (possibly including the current one)
    const better = biggerIsBetter(config.tool) ? (a, b) => a > b : (a, b) => a < b;
    const prevBest = {};
